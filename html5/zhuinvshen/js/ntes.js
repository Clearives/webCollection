...
function neteaseTracker(k, a, l, j) {
	if (is_spider()) {
		return
	}
	var d = j || _ntes_nacc;
	_ntes_nurl = escape(a || document.location);
	_ntes_ntit = escape(l || document.title);
	_ntes_nref = k === true ? "" : escape(document.referrer);
	...
	var c = 0;
	for (var f = 0; f < _ntes_domain_array.length; f++) {
		if (_ntes_cdmn == "." + _ntes_domain_array[f]) {
			c = 1;
			break
		}
	}
	if (c == 0) {
		for (f = 0; f < _non_ntes_domain_array.length; f++) {
			if (_non_ntes_cdmn == "." + _non_ntes_domain_array[f]) {
				c = 1;
	...
	if (c == 1) {
		var g = ntes_get_cookie("_ntes_nnid");
		if (g == -1) {
			...
			_ntes_nvid = g.substr(0, n);
			_ntes_surv = g.substr(n + 1, o - n - 1);
			if (_ntes_surv == 0) {
		...
		var m = ["_nacc=", d, "&_nvid=", _ntes_nvid, "&_nvtm=", _ntes_nvtm, "&_nvsf=", _ntes_nvsf, "&_nvfi=", _ntes_nvfi, "&_nlag=", _ntes_nlag, "&_nlmf=", _ntes_nlmf, "&_nres=", _ntes_nres, "&_nscd=", _ntes_nscd, "&_nstm=", _ntes_nstm, "&_nurl=", _ntes_nurl, "&_ntit=", _ntes_ntit, "&_nref=", _ntes_nref, "&_nfla=", _ntes_flsh, "&_nssn=", _ntes_nssn, "&_nxkey=", (b + "" + Math.random()).substring(6, 20), "&_end1"].join("");
		_ntes_nvsf = 0;
		_ntes_sendInfo("base", _ntes_src_addr + "/ntes?" + m, neteaseTracker.callback);
		neteaseTracker.callback = null;
		if (1) {
			var h = /(?:iPhone|iPad|iPod|Android|Adr |Windows Phone)/i;
			if (_ntes_nacc != "analytics" && _ntes_nacc != "research" && _ntes_nacc != "siteinfo" && _ntes_nurl.indexOf("https") != 0 && !h.test(navigator.userAgent)) {
				ntes_survey_popup()
			}
		}
	}
